...
class VerifyPreviousPinViewModel(private val userManager: UserManager, private val actions: Actions) : PinBaseViewModel() {
  fun onPinSubmit() {
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        onError()
        e.toastAndLog("VerifyPreviousPinViewModel")
      } finally {
        pin.value = ""
      }
    }
